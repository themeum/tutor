		...
		if ( version_compare( TUTOR_VERSION, '3.8.0', '>=' ) ) {
			...
			if ( ! $scheduler_table ) {
				$table_schema = "CREATE TABLE {$wpdb->prefix}tutor_scheduler (
					...
					scheduled_by BIGINT UNSIGNED NOT NULL COMMENT 'User who scheduled the action',
					scheduled_for BIGINT UNSIGNED NOT NULL COMMENT 'Target user of the scheduled action',
					PRIMARY KEY (id),

...
class Students_List extends \Dozent_List_Table {
	...
	function column_completed_course($item){
		...
		$courses = dozent_utils()->get_courses_by_user($user_id);
	...
	function get_columns(){
		$columns = array(
			...
			'completed_course'  => __('Completed Course', 'dozent'),
		);
		return $columns;
	}
	...
	function prepare_items() {
		...
		$total_items = dozent_utils()->get_total_students($search_term);
		$this->items = dozent_utils()->get_students(($current_page-1)*$per_page, $per_page, $search_term);;

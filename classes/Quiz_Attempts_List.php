...
class Quiz_Attempts_List extends \Dozent_List_Table {
	...
	function column_student($item){
		...
		$actions['answer'] = sprintf('<a href="?page=%s&sub_page=%s&attempt_id=%s">'.__('Review', 'dozent').'</a>',$_REQUEST['page'],'view_attempt',$item->comment_ID);
		...
		$quiz_title = '<strong>'.$item->display_name.'</strong> <br />'.$item->user_email.'<br /><br />'. human_time_diff(strtotime
			($item->comment_date)).__(' ago', 'dozent');
	...
	function column_course($item) {
		$quiz = dozent_utils()->get_course_by_quiz($item->comment_post_ID);
	...
	function column_earned_marks($item){
		...
		$marks_earned = dozent_utils()->avalue_dot('marks_earned', $attempt_info);
		$earned_percentage = $marks_earned > 0 ? ( number_format(($marks_earned * 100) / $total_marks)) : 0;

		$pass_mark_percent = dozent_utils()->avalue_dot('pass_mark_percent', $attempt_info);

		$output = $marks_earned." out of {$total_marks} ({$earned_percentage}%)  ";
		if ($earned_percentage >= $pass_mark_percent){
			$output .= '<span class="result-pass">'.__('Pass', 'dozent').'</span>';
		}else{
			$output .= '<span class="result-fail">'.__('Fail', 'dozent').'</span>';
		}
	...
	function column_attempt_status($item){
		...
		return  "<span class='dozent-status-context {$item->attempt_status}'>{$status}</span>";
	}
	...
	function get_columns(){
		$columns = array(
			...
			'student'           => __('Students', 'dozent'),
			'quiz'              => __('Quiz', 'dozent'),
			'course'            => __('Course', 'dozent'),
			'total_questions'   => __('Total Questions', 'dozent'),
			'earned_marks'      => __('Earned Mark', 'dozent'),
			'attempt_status'      => __('Earned Mark', 'dozent'),
		);
		return $columns;
	}
	...
	function prepare_items() {
		...
		$total_items = dozent_utils()->get_total_quiz_attempts($search_term);
		$this->items = dozent_utils()->get_quiz_attempts(($current_page-1)*$per_page, $per_page, $search_term);

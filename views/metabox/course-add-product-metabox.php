...
<div class="tutor-option-field-row">
	...
	<div class="tutor-option-field">
		...
		<select name="_tutor_course_product_id" class="tutor_select2" style="min-width: 300px;">
			...
			foreach ( $products as $product ) {
				if ( $product->ID == $product_id ) {
					echo _esc_html( '<option value="' . esc_attr( $product->ID ) . '" ' . selected( $product->ID, $product_id ) . ' >' . esc_attr( $product->post_title ) . '</option>' );
				}
		...
		<p class="desc">
			<a href="<?php echo esc_url( get_edit_post_link( $product_id ) ); ?>" target="_blank"><?php _e( 'Edit attached Product', 'tutor' ); ?></a> <br />

		...
		<select name="tutor_option[<?php echo esc_attr( $field_key ); ?>]" class="tutor-form-select">
			...
			if ( ! empty( $field['options'] ) ) {
				foreach ( $field['options'] as $option_key => $option ) {
					?>
					<option data-false="<?php echo $this->get( $field['key'], ( isset( $field['default'] ))); ?>" value="<?php echo esc_attr( $option_key ); ?>" <?php selected( $this->get( $field['key'], ( isset( $field['default'] ) ?  $field['default']  : null ) ), $option_key ); ?>><?php echo  esc_attr($option); ?></option>
					<?php
				}
			}
			?>

    ...
    <form id="tutor-option-form" class="tutor-option-form" method="post" data-toast_success_message="<?php _e('Settings Saved', 'tutor'); ?>">
        ...
        if (is_array($options_attr) && count($options_attr)) {
            ...
            foreach ($options_attr as $key => $option_group){
                ...
                echo "<li class='option-nav-item ".esc_attr($current_class)."'><a href='".esc_url($nav_url)."' data-tab='#".esc_attr($key)."' class='tutor-option-nav-item'>".esc_attr($option_group['label'])."</a> </li>";
            }
            ?>
            </ul>
            ...
            foreach ($options_attr as $key => $option_group){
                ...
                <div id="<?php echo esc_attr($key); ?>" class="tutor-option-nav-page <?php echo esc_attr($current_page) ? 'current-page' : ''; ?> " style="display: <?php echo $current_page ? 'block' : 'none' ?>;" >

...
<?php if ($data['bulk_action']): ?>
  ...
  <form action="" method="post" id="tutor-admin-bulk-action-form">
    ...
    <div class="tutor-form-select-with-btn">
      <select
        ...
        class="tutor-form-select tutor-form-control-sm"  name="bulk-action" id="tutor-admin-bulk-action"
      >
  ...
  <?php
		...
		<?php if (isset($data['filters']) && true === $data['filters']): ?>
			<?php
        $courses = (current_user_can('administrator')) ? tutils()->get_courses() : tutils()
...
?>
				...
				<div class="tutor-input-group tutor-form-control-sm">
					...
					placeholder="dd/mm/yyyy" id="tutor-backend-filter-date" name="tutor-backend-filter-date"
					/>
				</div>
				</div>

...
export interface OverviewChartColors {
...
export interface StatCardColors extends ChartColors {
  tooltip: TooltipColors;
}

export type CourseCompletionChartDataKey = 'enrolled' | 'completed' | 'in_progress' | 'inactive' | 'cancelled';

export type CourseCompletionChartData = {
...
const extractOverviewChartColors = (element: HTMLElement): OverviewChartColors => {
  return {
    earnings: {
      ...
      linearGradient: ['#5E81F4', getCSSProperty(element, '--tutor-surface-l1')],
    },
    enrolled: {
...
const createLinearGradient = (
  ...
  colorTop: string,
  colorBottom: string = getCSSProperty(document.documentElement, '--tutor-surface-l1'),
): CanvasGradient => {
...
export const statCard = (data: number[]) => ({
...
export const courseCompletionChart = (data: CourseCompletionChartData) => ({
  $el: null as HTMLDivElement | null,
  $refs: {} as { canvas: HTMLCanvasElement },

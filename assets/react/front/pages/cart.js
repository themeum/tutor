		...
		if (button) {
			...
			try {
				...
				if (status_code === 201) {
					...
					const viewCartButton = `<a id="tutor-native-view-cart" href="${data?.cart_page_url ?? "#"}" class="tutor-btn tutor-btn-outline-primary ${isSinglePage ? 'tutor-btn-lg tutor-btn-block' : 'tutor-btn-md'} ${!data?.cart_page_url ? 'tutor-cart-page-not-configured' : ''}">${__('View Cart', 'tutor')}</a>`

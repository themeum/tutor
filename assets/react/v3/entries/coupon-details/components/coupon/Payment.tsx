...
import type { PaymentStatus } from '@CouponServices/coupon';
		...
		<Box bordered>
			<BoxTitle>
				<div css={styles.paymentTitle}>
					...
					<PaymentBadge status={coupon.payment_status} />
				</div>
			</BoxTitle>
			<div css={styles.content}>
				<Box bordered>
					<div css={styles.item({ action: 'regular' })}>
						...
						<div>
							{coupon.courses.length} {__('Items', 'tutor')}
						</div>
						<div>{formatPrice(coupon.subtotal_price)}</div>
					</div>
					...
					<div css={styles.item({ action: 'bold' })}>
						...
						<div />
						<div>{formatPrice(coupon.total_price)}</div>
					</div>

					<Show when={coupon.refunds.length > 0}>
						<div css={styles.separator} />

						<Show when={coupon.refunds}>
							{(refunds) => (
								<For each={refunds}>
									{(refund, index) => (
						...
						<div css={styles.item({ action: 'bold' })}>
							...
							<div />
							<div>{formatPrice(coupon.net_total_price)}</div>
						</div>
					</Show>
				</Box>

				<div css={styles.markAsPaid}>
					<PaymentActionButton status={coupon.payment_status} onClick={() => alert('@TODO: will be handled later.')} />
				</div>
			</div>
		</Box>
	);

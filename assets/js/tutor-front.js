jQuery(document).ready(function ($) {
	...
	$(document).on(
		...
		function () {
			$(this)
				.closest(".tutor-star-rating-group")
				.find("i")
				.removeClass("tutor-icon-star-full")
			...
			for (var i = 1; i <= currentRateValue; i++) {
				$(this)
					...
					.addClass("tutor-icon-star-full");
			}
			$(this)
				.closest(".tutor-star-rating-group")
				.find('input[name="tutor_rating_gen_input"]')
				.val(currentRateValue);
		}
	);
	...
	$(document).on("focus", ".tutor_add_answer_textarea", function (e) {
		...
		var conf = {
			tinymce: {
				...
				toolbar1:
					"bold italic underline bullist strikethrough numlist  blockquote  alignleft aligncenter alignright undo redo link unlink spellchecker fullscreen",
			},
		};
	...
	$(document).on("click", ".tutor_cancel_wp_editor", function (e) {
		...
		$(this)
			.closest(".tutor_add_answer_wrap")
			.find(".tutor_wp_editor_show_btn_wrap")
			.toggle();
		var question_id = $(this)
			.closest(".tutor_add_answer_wrap")
			.attr("data-question-id");
	...
	if ($tutor_quiz_time_update.length) {
		attempt_settings = JSON.parse(
			$tutor_quiz_time_update.attr("data-attempt-settings")
		);
		var attempt_meta = JSON.parse(
			$tutor_quiz_time_update.attr("data-attempt-meta")
		);

		if (attempt_meta.time_limit.time_limit_seconds > 0) {
			//No time Zero limit for
			var countDownDate =
				new Date(attempt_settings.attempt_started_at).getTime() +
			...
			var tutor_quiz_interval = setInterval(function () {
				...
				if (distance < 0) {
					...
					if (
						_tutorobject.quiz_options.quiz_when_time_expires === "autosubmit"
					) {
						/**
						 * Auto Submit
						 */
						...
						_tutorobject.quiz_options.quiz_when_time_expires === "autoabandon"
					) {
						/**
						 *
						 * @type {jQuery}
						...
						var tutor_quiz_remaining_time_secs = $(
							"#tutor_quiz_remaining_time_secs"
						).val();
	...
	$(document).on(
		"click",
		".tutor-topics-title h3 .toogle-informaiton-icon",
		function (e) {
			$(this)
	...
	function init_quiz_builder() {
		if (jQuery().sortable) {
			...
			$(".quiz-draggable-rand-answers, .quiz-answer-matching-droppable")
				.sortable({
	...
	$(document).on("click", ".tutor-quiz-answer-next-btn", function (e) {
		...
		var question_id = parseInt(
			$that.closest(".quiz-attempt-single-question").attr("id").match(/\d+/)[0],
			10
		);

		var next_question_id = $that
		...
		if (next_question_id) {
			...
			if ($nextQuestion && $nextQuestion.length) {
				...
				if ($(".tutor-quiz-questions-pagination").length) {
					...
					$(
						'.tutor-quiz-questions-pagination a[href="' +
							next_question_id +
							'"]'
					).addClass("active");
				}
	...
	function tutor_quiz_validation($question_wrap) {
		...
		if ($required_answer_wrap.length) {
			...
			if ($inputs.length) {
				...
				if ($type === "radio") {
					if (
						$required_answer_wrap.find('input[type="radio"]:checked').length ==
						0
					) {
						$question_wrap
							.find(".answer-help-block")
							.html(
								'<p style="color: #dc3545">Please select an option to answer</p>'
							);
						validated = false;
					}
				} else if ($type === "checkbox") {
					if (
						$required_answer_wrap.find('input[type="checkbox"]:checked')
							.length == 0
					) {
						$question_wrap
							.find(".answer-help-block")
							.html(
								'<p style="color: #dc3545">Please select at least one option to answer.</p>'
							);
						validated = false;
					}
				} else if ($type === "text") {
					...
					$inputs.each(function (index, input) {
						if (!$(input).val().trim().length) {
							$question_wrap
								.find(".answer-help-block")
								.html(
									'<p style="color: #dc3545">The answer for this question is required</p>'
								);
							validated = false;
						}
					});
			 ...
			 * Matching Question
			 */
			var $matchingDropable = $required_answer_wrap.find(
			...
			if ($matchingDropable.length) {
				$matchingDropable.each(function (index, matching) {
					if (!$(matching).find(".quiz-draggable-answer-item").length) {
						$question_wrap
							.find(".answer-help-block")
							.html('<p style="color: #dc3545">Please match all the items</p>');
						validated = false;
					}
				});
			}
	...
	if ($.fn.ShareLink) {
		...
		if ($social_share_wrap.length) {
			var share_config = JSON.parse(
				$social_share_wrap.attr("data-social-share-config")
			);
	...
	$(".withdraw-method-select-input").on("change", function (e) {
		...
		$(
			"#withdraw-method-form-" +
				$that.closest(".withdraw-method-select").attr("data-withdraw-method")
		).show();
	});
	...
	$(document).on("submit", "#tutor-withdraw-account-set-form", function (e) {
		...
		$.ajax({
			...
			success: function (data) {
				if (data.success) {
					var successMsg =
						'<div class="tutor-success-msg" style="display: none;"><i class="tutor-icon-mark"></i> ' +
						data.data.msg +
						" </div>";
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.success) {
					...
					Msg =
						'<div class="tutor-success-msg"><i class="tutor-icon-mark"></i> ' +
						data.data.msg +
						" </div>";
				} else {
					Msg =
						'<div class="tutor-error-msg"><i class="tutor-icon-line-cross"></i> ' +
						data.data.msg +
						" </div>";
				}
	...
	$(document).on(
		"click",
		".tm-close, .tutor-frontend-modal-overlay, .tutor-modal-btn-cancel",
		function () {
	...
	$(document).on("click", ".tutor-profile-photo-delete-btn", function () {
		$(".tutor-profile-photo-upload-wrap")
			.find("img")
			.attr("src", _tutorobject.placeholder_img_src);
	...
	$(document).on("submit", "#tutor_assignment_submit_form", function (e) {
		...
		if (assignment_answer.trim().length < 1) {
			$("#form_validation_response").html(
				'<div class="tutor-error-msg">' +
					_tutorobject.text.assignment_text_validation_msg +
					"</div>"
			);
			e.preventDefault();
		}
	...
	$(document).on(
		"click",
		".video_source_upload_wrap_html5 .video_upload_btn",
		function (event) {
			...
			frame.on("select", function () {
				...
				$that
					.closest(".video_source_upload_wrap_html5")
		...
		frame.on("select", function () {
			...
			if (attachments.length) {
				for (var i = 0; i < attachments.length; i++) {
					...
					var inputHtml =
						'<div class="tutor-added-attachment"><i class="tutor-icon-archive"></i><a href="javascript:;" class="tutor-delete-attachment tutor-icon-line-cross"></a> <span> <a href="' +
						attachment.url +
						'">' +
						attachment.filename +
						'</a> </span> <input type="hidden" name="tutor_attachments[]" value="' +
						attachment.id +
						'"></div>';
					$that
						.closest(".tutor-lesson-attachments-metabox")
						.find(".tutor-added-attachments-wrap")
						.append(inputHtml);
				}
			}
		});
		...
		$.ajax({
			...
			success: function (data) {
				...
				tinymce.execCommand(
					"mceRemoveEditor",
					false,
					"tutor_assignments_modal_editor"
				);
				tinyMCE.execCommand(
					"mceAddEditor",
					false,
	...
	$(".tutor-course-builder-section-title").on("click", function () {
		if ($(this).find("i").hasClass("tutor-icon-up")) {
			$(this)
				.find("i")
				.removeClass("tutor-icon-up")
				.addClass("tutor-icon-down");
		} else {
			$(this)
				.find("i")
				.removeClass("tutor-icon-down")
				.addClass("tutor-icon-up");
		}
		...
		$.ajax({
			...
			success: function (data) {
				if (typeof data.data !== "undefined") {
					$(".tutor-edit-review-modal-wrap .modal-container").html(
						data.data.output
					);
					$(".tutor-edit-review-modal-wrap")
						.attr("data-review-id", review_id)
						.addClass("show");
				}
			},
			complete: function () {
	...
	$(document).on("submit", "#tutor_update_review_form", function (e) {
		...
		var $that = $(this);
		var review_id = $that
			.closest(".tutor-edit-review-modal-wrap ")
			.attr("data-review-id");
		...
		$.ajax({
			...
			complete: function () {
				$that
					...
					.removeClass("tutor-updating-message");
			},
		});
	});

...
class Paypal extends BasePayment {
	...
	protected function getAccessToken(): string {
		if ( empty( $this->accessToken ) ) {
			...
			$requestData = (object) array(
				...
				'options' => array(
					'auth'        => array( $this->config->get( 'client_id' ), $this->config->get( 'client_secret' ) ),
	...
	public function verifyAndCreateOrderData( object $payload ): object {
		try {
			...
			$this->webhook_signature_validation( $payload );
	...
	public function createRecurringPayment() {
		try {
			...
			if ( $responseData->status === 'COMPLETED' ) {
				...
				$responseData->payment_source->paypal->attributes = (object) array(
					'vault' => (object) array(
	...
	private function webhook_signature_validation( $payload ) {
		$payload_stream = json_decode( $payload->stream );
		$data           = array(
			'auth_algo'         => $payload->server['PAYPAL-AUTH-ALGO'],
			...
			'webhook_id'        => $this->config->get( 'webhook_id' ),
			'webhook_event'     => array(
				'event_version'    => $payload_stream->event_version,
				'resource_version' => $payload_stream->resource_version,
			),
		);

		$requestData = (object) array(
			'method'  => 'post',
			'url'     => $this->config->get( 'api_url' ) . '/v1/notifications/verify-webhook-signature',
			'options' => array(
				'headers' => $this->headers,

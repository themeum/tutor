...
describe("Tutor Admin Instructors", () => {
    ...
    it ("should update a instructor successfully", () => {
        ...
        cy.setTinyMceContent(".tutor-instructor-edit-modal.tutor-is-active .wp-editor-container", "This is an updated bio.")
        ...
        cy.get("body").then(($body) => {
            cy.get(".tutor-table tbody tr").eq(0).then(($row) => {
                for (let index = 0; index < 2; index++) {
                    cy.wrap($row).find(".tutor-table-row-status-update").invoke('val').then((status) => {
                        if (status !== 'pending') {
    ...
    it ("should visit a instructor profile successfully", () => {
        cy.get(".tutor-table tbody tr").eq(0).find("td").eq(1).then(($data) => {
            cy.wrap($data).find("a").invoke("attr", "href").then((link) => cy.visit(link))

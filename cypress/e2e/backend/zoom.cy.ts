...
describe('Tutor Dashboard My Courses', () => {
  ...
  it('should delete a zoom meeting', () => {
    ...
    cy.get('body').then(($body) => {
      if ($body.text().includes('No Data Found.')) {
        ...
        cy.get('button').contains('Yes, Delete This').click({ force: true });
  ...
  it('should allow users to check checkboxes', () => {
    ...
    // recording options
    cy.get('input#tutor_zoom_rec_local').check().should('be.checked');
    cy.get('input#tutor_zoom_rec_none').should('not.be.checked');
    cy.get('input#tutor_zoom_rec_cloud').should('not.be.checked');
    cy.get('input#tutor_zoom_rec_cloud').check().should('be.checked');
    cy.get('input#tutor_zoom_rec_none').should('not.be.checked');

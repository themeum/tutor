        ...
        cy.get("body").then(($body) => {
            if ($body.text().includes("No Data Available in this Section")) {
              ...
              cy.get('.tutor-form-select-option > span').then($options => {
                const filteredOptions = $options.filter((index, option) => option.getAttribute('title') !== "Select a course/bundle");
                ...
                cy.wrap(filteredOptions[randomIndex]).click({force:true}).then(() => {
...
//     cy.get(':nth-child(2) > .tutor-js-form-select > .tutor-form-select-dropdown > .tutor-form-select-options').contains(selectedOptionText)
//       .click({ force: true });
...
//     const itemSelector =
//       ".tutor-d-flex.tutor-align-center.tutor-gap-2";

...
describe("Tutor Admin Course Bundles", () => {
    ...
    it ("should create a bundle course successfully", () => {
        cy.get(".tutor-wp-dashboard-header a").contains("Add New").click()

        cy.getByInputName("post_title").type("My new bundle course")
        cy.setTinyMceContent("#wp-content-editor-container", "Our Digital Marketing Mastery bundle covers vital areas like social media strategies, SEO, email marketing, content creation, and paid advertising on platforms like Google and Facebook.")
        ...
        cy.get("#sample-permalink a").invoke("attr", "href").then((link) => {
            cy.get("a").contains("View post").click()
    ...
    it ("should update a bundle course successfully", () => {
        ...
        cy.get("#tutor-bundle-course-list-wrapper").then(($courses) => {
            if (!$courses.text().includes("Select courses to see overview")) {
                cy.get(".tutor-card.tutor-course-card .tutor-bundle-course-delete a").eq(0).click({ force: true })
                cy.wait("@curseRemoveAjax").then((interception) => {
                    expect(interception.response.body.success).to.equal(true);
                });
            }
        ...
        cy.get("body").then(($body) => {
            if ($body.text().includes("No Data Available in this Section")) {
                ...
                cy.get(".tutor-table tbody tr").then(($rows) => {
                    ...
                    cy.wrap($rows[randomNumber]).find("a").contains("View").invoke('attr', 'href').then((link) => {
                        cy.visit(link)
    ...
    it("should change a bundle products status", () => {
        ...
        cy.get("body").then(($body) => {
            if ($body.text().includes("No Data Available in this Section")) {
                ...
                cy.get(".tutor-table tbody tr").eq(0).then(($row) => {
                    cy.wrap($row).find("button[action-tutor-dropdown=toggle]").click()

                    for (let index = 0; index < 2; index++) {
                        cy.wrap($row).find(".tutor-table-row-status-update").invoke("attr", "data-status").then((status) => {
                            if (status !== 'draft') {
    ...
    it("should delete a bundle product successfully", () => {
        ...
        cy.get("body").then(($body) => {
            if ($body.text().includes("No Data Available in this Section")) {
                ...
                cy.get(".tutor-modal.tutor-is-active button").contains("Yes, I'am Sure").click()

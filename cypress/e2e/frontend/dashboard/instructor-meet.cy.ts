...
describe("Tutor Dashboard My Courses", () => {
  ...
  it("Should filter courses by a specific date", () => {
      cy.get('.tutor-wp-dashboard-filter-items > :nth-child(3) > .tutor-v2-date-picker > .tutor-react-datepicker > .react-datepicker-wrapper > .react-datepicker__input-container > .tutor-form-wrap > .tutor-form-control').click();
  ...
  it("should upload meet integration json and save connection", () => {
    const filePath = "cypress/fixtures/google-meet-integration.json";
    cy.get("body").then(($body) => {
    ...
    it("should create new course and google meeting", () => {
      ...
      cy.get(
        "textarea[name='meeting_summary'"
      ).type("Google meet test summary", { force: true });

      cy.get('.tutor-gmi-meeting-time > :nth-child(1) > .tutor-v2-date-picker > .tutor-react-datepicker > .react-datepicker-wrapper > .react-datepicker__input-container > .tutor-form-wrap > .tutor-form-control').click();
      ...
      cy.get(".react-datepicker__day")
        ...
        cy.get(".react-datepicker__day")
          ...
          cy.get('.tutor-col-md-8 > .tutor-js-form-select > .tutor-form-select-dropdown > .tutor-form-select-options > :nth-child(108) > .tutor-nowrap-ellipsis').click();

      cy.get("input[name='meeting_attendees_enroll_students']").check()

      cy.get(
        ".tutor-gm-create-new-meeting"
      )
    ...
    it("should start meeting", () => {
      ...
      cy.get("a.tutor-btn.tutor-btn-primary")
        ...
        .click().then((url)=>{console.log(url)})
    ...
    it("should upload meet integration json dnd and save connection",()=>{
      const filePath = 'cypress/fixtures/google-meet-integration.json';
      cy.get('#tutor-google-meet-credential-upload').selectFile(filePath, { action: 'drag-drop' ,force:true});

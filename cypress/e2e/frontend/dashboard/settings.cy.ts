...
describe("Tutor Dashboard Settings", () => {
    ...
    //     cy.getByInputName("previous_password").type(Cypress.env("admin_password"))
    //     cy.getByInputName("new_password").type(Cypress.env("admin_password"))
    //     cy.getByInputName("confirm_new_password").type(Cypress.env("admin_password"))
    ...
    //         if ($body.text().includes("Withdraw")) {
    //             cy.get("a.tutor-nav-link").contains("Withdraw").click()
                
    //             cy.getByInputName("previous_password").clear().type("John Due")

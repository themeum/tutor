    ...
    cy.get(
      ...
      cy.get('body').then(($body) => {
        if ($body.text().includes('No Data Found from your Search/Filter')) {
          ...
          cy.get('.tutor-fs-7>a').each(($announcement) => {
              ...
              .then((link) => {
                ...
                cy.get('body').then(($body) => {
                  if ($body.text().includes('Evaluate')) {
                    cy.get('.tutor-btn.tutor-btn-primary.tutor-mt-16').contains('Evaluate').click();
                    ...
                    cy.url().then((url) => {
                      ...
                      cy.get('.submitted-assignment-title > .tutor-btn').contains('Back').click();
                      cy.get('.tutor-mb-24 > .tutor-btn').contains('Back').click();
                    });
                  } else {
                    cy.get('.tutor-mb-24 > .tutor-btn').contains('Back').click();
                  }

describe('Tutor Student Paid Course Journey', () => {
  ...
  it('should be able to enroll in a paid course, view cart, and manage items', () => {
    ...
    cy.isEnrolled().then((isEnrolled) => {
      if (!isEnrolled) {
        cy.get("button[name='add-to-cart']").contains('Add to cart').click();
        // Login as a student
        ...
        cy.get('body').then(($body) => {
          if ($body.find("button[name='add-to-cart']").length > 0) {
            ...
            cy.get('.tutor-woocommerce-view-cart').contains('View Cart').click();
          }
        });

        cy.url().then((url) => {
          if (url.includes('/cart')) {
            cy.get('.wc-block-cart__submit-button').contains('Proceed to Checkout').click();
            cy.url().should('include', '/checkout');

            cy.get('#billing-first_name').clear().type('Student');
            ...
            cy.get('#components-form-token-input-1').clear().type('Florida');

            cy.get('#billing-postcode').clear().type('96799');
            cy.get('#billing-phone').clear().type('+8801555123456');

            cy.get(':nth-child(4) > .wc-block-components-radio-control__option').click();

...
<div class="tutor-option-field-row">
    <div class="tutor-option-field-label">
        <label for=""><?php _e('Public Profile Laout', 'tutor'); ?></label>
    </div>
        ...
        <div class="instructor-layout-templates-fields">
			<?php
				...
				foreach ($profile_templates as $template){
				    ...
				    $selected_template = tutor_utils()->get_option('public_profile_layout');
					?>
                    <label class="instructor-layout-template <?php echo ($template === $selected_template) ? 'selected-template' : '' ?> ">
                        <img src="<?php echo $img; ?>" />
                        <input type="radio" name="tutor_option[public_profile_layout]" value="<?php echo $template; ?>" <?php checked($template, $selected_template) ?> style="display: none;" >
                    </label>
					<?php
				}
			?>

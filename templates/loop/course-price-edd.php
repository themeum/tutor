...
add_filter(
	...
	function( $defaults ) {
		if ( isset( $defaults['class'] ) ) {
			...
			$defaults['class'] = 'edd-add-to-cart button white edd-submit edd-has-js ' . apply_filters( 'tutor_course_loop_button_class', 'tutor-btn tutor-btn-outline-primary tutor-btn-md tutor-btn-block', $course_id ) . ' ';
		}
		return $defaults;
	}
);
...
<?php
	$course_id  = get_the_ID();
	$enroll_btn = '<div class="list-item-button">' . apply_filters( 'tutor_course_restrict_new_entry', '<a href="' . get_the_permalink() . '" class="' . apply_filters( 'tutor_course_loop_button_class', 'tutor-btn tutor-btn-outline-primary tutor-btn-md tutor-btn-block', $course_id ) . '">' . __( 'Enroll Course', 'tutor' ) . '</a>', $course_id ) . '</div>';

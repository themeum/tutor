	...
	if ( ! empty( $answers ) ) {
		...
		foreach ( $answers as $key => $ans ) {
			...
			switch ( $type ) {
				case 'text_image':
					...
					if ( isset( $ans->answer_title ) ) {
						echo _esc_html( '<span class="caption">' . esc_attr( stripslashes( $ans->answer_title ) ) . '</span>' );
					}
					echo _esc_html( '</div>' );
					break;
				case 'text':
					...
					if ( isset( $ans->answer_title ) ) {
						echo _esc_html( '<span class="text-item-caption">' . esc_attr( stripslashes( $ans->answer_title ) ) . '</span>' );
					}
					echo _esc_html( '</div>' );
					break;
				case 'image':
			...
			if ( isset( $ans->answer_two_gap_match ) ) {
				...
				echo _esc_html( '<div class="image-match">' . esc_attr( stripslashes( $ans->answer_two_gap_match ) ) . '</div>' );
				echo _esc_html( '</div>' );
			}
		}
		echo _esc_html( '</div>' );
...
<div class="tutor-quiz-attempt-review-wrap">
	...
	<table class="wp-list-table">
		...
		<tr>
			<td><?php echo esc_attr( $attempt_data->attempt_id ); ?></td>
			<td>
				<?php
			...
			<td><?php echo esc_attr( $attempt_data->total_questions ); ?></td>
			<td><?php echo esc_attr( $attempt_data->total_marks ); ?></td>
			<td>
				<?php
					$pass_mark_percent = tutor_utils()->get_quiz_option( $attempt_data->quiz_id, 'passing_grade', 0 );
					echo esc_attr( $pass_mark_percent ) . '%';
				?>
			</td>
			<td>
				<?php
	...
	<div class="tutor-quiz-attempt-review-wrap">
		<div class="quiz-attempt-answers-wrap">
			...
			<div class="instructor-feedback-content">
				<p><?php echo _esc_html( $feedback ); ?></p>
			</div>
		</div>
	</div>
<?php } ?>
						...
						<td>
							...
							if ( $answer->question_type === 'true_false' || $answer->question_type === 'single_choice' ) {
								...
								if ( $count_dash_fields ) {
									...
									echo str_replace( '{dash}', '_____', esc_attr( stripslashes( $answer_title ) ) );
								}
							} elseif ( $answer->question_type === 'open_ended' || $answer->question_type === 'short_answer' ) {

								if ( $answer->given_answer ) {
									echo _esc_html( wpautop( stripslashes( $answer->given_answer ) ) );
								}
								...
								foreach ( $original_saved_answers as $key => $original_saved_answer ) {
									...
									if ( tutils()->count( $provided_answer_order ) ) {
										...
										echo esc_attr( stripslashes( $original_saved_answer->answer_title ) ) . ' - ' . esc_attr( stripslashes( $provided_answer_order->answer_two_gap_match ) ) . '<br />';
									}
								}
								...
								foreach ( $original_saved_answers as $key => $original_saved_answer ) {
									...
									<div class="image-matching-item">
										...
										<p class="dragged-caption"><?php echo esc_attr( stripslashes( $provided_answer_order->answer_title ) ); ?></p>
									</div>
									<?php
								}
								echo _esc_html('</div>';)
						...
						<td>
							...
							if ( ( $answer->question_type != 'open_ended' && $answer->question_type != 'short_answer' ) ) {
								...
								if ( $answer->question_type === 'true_false' ) {
									...
									if ( $correct_answer ) {
										echo implode( ', ', explode( '|', esc_attr( stripslashes( $correct_answer ) ) ) );
									}
								} elseif ( $answer->question_type === 'ordering' ) {

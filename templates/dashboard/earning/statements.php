...
?>
	<div class="tutor-dashboard-inline-links">
		<ul>
			<li><a href="<?php echo esc_url( tutor_utils()->get_tutor_dashboard_page_permalink( 'earning' ) ); ?>"> <?php _e( 'Earning' ); ?></a>
			</li>
			<li><a href="<?php echo esc_url( tutor_utils()->get_tutor_dashboard_page_permalink( 'earning/report' ) ); ?>"> <?php _e( 'Report', 'tutor' ); ?> </a></li>
			<li class="active">
...
if ( $statements->count ) {
	...
	<p class="tutor-dashboard-pagination-results-stats">
		...
		<?php
		echo sprintf( __( 'Showing results %1$d to %2$d of %3$d', 'tutor' ), $offset + 1, min( $statements->count, $offset + 1 + tutor_utils()->count( $statements->results ) ), $statements->count );
		?>
	</p>
		...
		<table class="tutor-dashboard-statement-table tutor-table">
			...
			foreach ( $statements->results as $statement ) {
				...
				<tr>
					...
					<td>
						<p><?php echo esc_attr( tutor_utils()->tutor_price( $statement->admin_amount ) ); ?> </p>
					...
					<td>
						<p><?php echo esc_attr( $statement->deduct_fees_name ); ?>  <?php echo esc_attr( tutor_utils()->tutor_price( $statement->deduct_fees_amount ) ); ?>
						</p>

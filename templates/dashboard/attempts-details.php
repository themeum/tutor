...
$previous_attempts = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}tutor_quiz_attempts WHERE user_id = {$user_id} AND attempt_id = {$attempt_id}");
$attempted_count = is_array($previous_attempts) ? count($previous_attempts) : 0;

echo '<pre>';
print_r( $previous_attempts );
echo '</pre>';
?>

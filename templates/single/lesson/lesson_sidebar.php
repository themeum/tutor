...
<?php do_action( 'tutor_lesson/single/before/lesson_sidebar' ); ?>
	...
	<div class="tutor-sidebar-tabs-wrap">
		<div class="tutor-tabs-btn-group">
			<a href="#tutor-lesson-sidebar-tab-content" class="<?php echo esc_attr( $enable_q_and_a_on_course ? 'active' : '' ); ?>"> <i class="tutor-icon-education"></i> <span> <?php esc_html_e( 'Lesson List', 'tutor' ); ?></span></a>
								...
								if ( $lessons->have_posts() ) {
									while ( $lessons->have_posts() ) {
										...
										if ( $post->post_type === 'tutor_quiz' ) {
											$quiz = $post;
											?>
											<div class="tutor-single-lesson-items quiz-single-item quiz-single-item-<?php echo esc_attr( $quiz->ID ); ?> <?php echo ( $currentPost->ID === get_the_ID() ) ? 'active' : ''; ?>" data-quiz-id="<?php echo esc_attr( $quiz->ID ); ?>">
												<a href="<?php echo get_permalink( $quiz->ID ); ?>" class="sidebar-single-quiz-a" data-quiz-id="<?php echo esc_attr( $quiz->ID ); ?>">
													<i class="tutor-icon-doubt"></i>
													<span class="lesson_title"><?php echo esc_attr( $quiz->post_title ); ?></span>
											...
											<div class="tutor-single-lesson-items assignments-single-item assignment-single-item-<?php echo esc_attr( $post->ID ); ?> <?php echo ( $currentPost->ID === get_the_ID() ) ? 'active' : ''; ?>" data-assignment-id="<?php echo esc_attr( $post->ID ); ?>">
												<a href="<?php echo get_permalink( $post->ID ); ?>" class="sidebar-single-assignment-a" data-assignment-id="<?php echo esc_attr( $post->ID ); ?>">
											...
											?>
											<div class="tutor-single-lesson-items zoom-meeting-single-item zoom-meeting-single-item-<?php echo esc_attr( $post->ID ); ?> <?php echo ( $currentPost->ID === get_the_ID() ) ? 'active' : ''; ?>"

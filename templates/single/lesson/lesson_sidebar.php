								...
								if ( $lessons->have_posts() ) {
									while ( $lessons->have_posts() ) {
										...
										if ( $post->post_type === 'tutor_quiz' ) {
											$quiz = $post;
											?>
											<div class="tutor-single-lesson-items quiz-single-item quiz-single-item-<?php echo esc_attr( $quiz->ID ); ?> <?php echo esc_attr( ( $currentPost->ID === get_the_ID() ) ? 'active' : '' ); ?>" data-quiz-id="<?php echo esc_attr( $quiz->ID ); ?>">

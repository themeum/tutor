			...
			<div class="tutor-accordion-item-body" style="<?php echo $is_topic_active ? ' display: block;' : 'display: none;'; ?>">
				...
				while ( $lessons->have_posts() ) {
					...
					if ( $post->post_type === 'tutor_quiz' ) {
						$quiz = $post;
						?>
						<div class="tutor-course-topic-item tutor-course-topic-item-quiz<?php echo ( $currentPost->ID == get_the_ID() ) ? ' is-active' : ''; ?>" data-quiz-id="<?php echo esc_attr( $quiz->ID ); ?>">
					...
					<?php } elseif ( $post->post_type === 'tutor_zoom_meeting' ) { ?>
						<div class="tutor-course-topic-item tutor-course-topic-item-zoom<?php echo esc_attr( ( $currentPost->ID == get_the_ID() ) ? ' is-active' : '' ); ?>">
					...
					<?php } elseif ( $post->post_type === 'tutor-google-meet' ) { ?>
						<div class="tutor-course-topic-item tutor-course-topic-item-zoom<?php echo esc_attr( $currentPost->ID == get_the_ID() ? ' is-active' : '' ); ?>">
							<a href="<?php echo $show_permalink ? esc_url( get_permalink( $post->ID ) ) : '#'; ?>">
								<div class="tutor-d-flex tutor-mr-32">
					...
					<?php } else { ?>
						...
						?>
						<div class="tutor-course-topic-item tutor-course-topic-item-lesson<?php echo esc_attr( $currentPost->ID == get_the_ID() ? ' is-active' : '' ); ?>">

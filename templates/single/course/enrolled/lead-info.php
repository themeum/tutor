...
<div class="dozent-single-course-segment dozent-single-course-lead-info">
    ...
    <div class="dozent-course-enrolled-info">
		<?php $count_completed_lesson = dozent_course_completing_progress_bar(); ?>

        <div class="dozent-lead-info-btn-group">
			<?php
			if ( $wp_query->query['post_type'] !== 'lesson') {
				$lesson_url = dozent_utils()->get_course_first_lesson();
				if ( $lesson_url ) {
					?>
                    <a href="<?php echo $lesson_url; ?>" class="dozent-button"><?php _e( 'Continue to lesson', 'dozent' ); ?></a>
                <?php }
            }
            ?>
            <?php dozent_course_mark_complete_html(); ?>
        </div>
    </div>

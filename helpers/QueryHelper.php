...
class QueryHelper {
	...
	public static function delete_comment_with_meta( array $where ) {
		...
		if ( is_array( $ids ) && count( $ids ) ) {
			...
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->commentmeta} WHERE comment_id IN({$in_clause}) ", $ids ) );//phpcs:ignore
			// delete comment.
	...
	public static function delete_post_with_meta( array $where ) {
		...
		if ( is_array( $ids ) && count( $ids ) ) {
			$in_clause = implode( ',', array_fill( 0, count( $ids ), '%d' ) );
			// delete post metas.
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->postmeta} WHERE post_id IN({$in_clause}) ", $ids ) );//phpcs:ignore
			// delete post.
